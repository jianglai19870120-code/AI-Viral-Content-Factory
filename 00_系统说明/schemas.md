...
- `copy-structure-v14.schema.json`：历史文案结构候选，只可查阅。
- `final-copy-v3.schema.json`：按完整 FNN 大框架生成的正文候选；结构四只冻结核心段，非核心段只使用框架功能定义补足，不含逐句复刻画像。
- `audit-receipt-v3.schema.json`：小审正式回执。
- `video-evidence-bundle-v1`：视频文案一次性原文证据包（仅运行期与审核使用，不是长期处理库资产）。
- `video-pain-card-v1`：带场景、表达与精确来源证据的视频痛点卡；观点与误区卡沿用同一证据包并各自保留来源链。

不得新建按干货、推荐、获客等内容类型分流的结构或正文合同。
