#!/usr/bin/env python3
"""Bind V3 authored FNN sections and render their companion annotations."""
...
def main()->int:
    ...
    if plan.get('schema')!='final-copy-plan-v3' or not isinstance(texts,dict):raise SystemExit('只接受完整 FNN 的 V3 正文计划与 JSON 段落映射')
    ...
    payload={'schema':'final-copy-v3','topic':plan['topic'],'benchmark_case_id':plan['benchmark_case_id'],'plan_sha256':digest(a.plan),'framework_mappings':mappings}
