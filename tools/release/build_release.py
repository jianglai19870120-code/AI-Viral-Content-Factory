...
def write_checksums(destination: Path) -> Path:
    ...
    for path in sorted(destination.rglob("*")):
        if path.is_file() and path != target and ".git" not in path.relative_to(destination).parts:
...
def create_member_asset_placeholders(root: Path, destination: Path) -> None:
    ...
    if not asset_root.is_dir():
        return
    member_directories: set[Path] = set()
    for source_dir in sorted(path for path in asset_root.rglob("*") if path.is_dir()):
        relative = source_dir.relative_to(root)
        if is_member_exclusive(relative, directory=True):
            member_directories.add(relative)
    for relative in sorted(member_directories):
...
def _portable_path(value: str, root: Path) -> str:
    ...
    if not ABSOLUTE_PATH.match(normalized):
        return value
    project_root = str(root.resolve()).replace("\\", "/").rstrip("/")
    if normalized.casefold().startswith(project_root.casefold() + "/"):
        return normalized[len(project_root) + 1:]
    filename = Path(normalized).name
    return f"external-source/{filename}" if filename else "external-source"


def _portable_json_value(value: object, root: Path, key: str | None = None) -> object:
    if isinstance(value, dict):
        return {name: _portable_json_value(item, root, str(name)) for name, item in value.items()}
    if isinstance(value, list):
        return [_portable_json_value(item, root, key) for item in value]
    if isinstance(value, str) and key:
        normalized_key = key.casefold()
        if normalized_key == "path" or normalized_key.endswith("path") or normalized_key.endswith("paths"):
            return _portable_path(value, root)
    return value


def sanitize_staged_paths(destination: Path, source_root: Path) -> None:
    """Remove local absolute paths from copied JSON and JSONL metadata only."""
    for path in destination.rglob("*"):
        ...
        if path.suffix.lower() == ".json":
            try:
                ...
                continue
            sanitized = _portable_json_value(payload, source_root)
...
def github_package(root: Path, destination: Path) -> None:
    ...
    for raw_skill in registry["skills"]:
        ...
        if skill.get("releaseStatus") != "member-exclusive" and not is_member_exclusive(skill["sourceDir"], directory=True):
            continue
    ...
    sanitize_staged_paths(destination, root)
    build_manifest(destination, destination / "PUBLIC_ASSETS_MANIFEST.json")
...
"""
    ...
    for raw_skill in registry["skills"]:
        ...
        if skill.get("releaseStatus") == "member-exclusive" or is_member_exclusive(skill["sourceDir"], directory=True):
            target.write_text(
