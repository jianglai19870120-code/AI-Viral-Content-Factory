...
def is_member_exclusive(relative: Path) -> bool:
    return any(MEMBER_EXCLUSIVE_SUFFIX in part for part in relative.parts)
...
def classify(relative: Path) -> tuple[str, str]:
    ...
    # 输入资料和图像生成工作流会携带原始内容、提示词或本机路径；公开包只提供
    # 目录合同与 Skill，不分发这些运行资产。
    if text.startswith("02_资产中心/01_输入库/"):
        return "private", "source-input-asset"
    if text.startswith("02_资产中心/02_处理库/") or text.startswith("02_资产中心/03_输出库/"):
        return "private", "local-content-asset"
    if text.startswith("02_资产中心/06_配图库/"):
        return "private", "local-generated-gallery"
    if text.startswith("03_工作台/01_品牌设计系统/字体/"):
        return "private", "licensed-local-font"
    if text.startswith("tools/tmp_"):
        return "private", "temporary-tooling"
    if text.startswith("02_资产中心/06_配图库/") and ("evidence" in parts or "prompts" in parts or relative.name.startswith("codex-render-") or relative.name == "codex-workflow.txt"):
...
def registered_member_roots(root: Path) -> tuple[set[Path], str | None]:
    """Read custom member roots so release checks do not rely on name suffixes."""
    registry = root / "03_工作台" / "config" / "content-types.json"
    ...
    try:
        ...
        for entry in entries:
            if not isinstance(entry, dict) or not entry.get("memberOnly"):
                continue
            values = entry.get("roots")
            if not isinstance(values, dict):
                raise ValueError("会员类型缺少三库目录")
            for value in values.values():
                if value is None:
                    continue
                relative = Path(str(value))
                if relative.is_absolute() or ".." in relative.parts:
                    raise ValueError("会员类型目录必须是资产中心内的相对路径")
                roots.add(Path("02_资产中心") / relative)
        return roots, None
    except (OSError, ValueError, json.JSONDecodeError) as exc:
        return set(), f"内容类型注册表无法用于发布校验：{exc}"


def release_violations(root: Path, *, check_index: bool = False) -> list[str]:
    load_policy(root)
    failures: list[str] = []
    member_roots, registry_error = registered_member_roots(root)
    if registry_error:
    ...
    for relative in tracked_files(root):
        ...
        if any(relative.is_relative_to(member_root) for member_root in member_roots) and relative.name != ".gitkeep":
            status, reason = "private", "registered-member-content"
        if status != "public":
