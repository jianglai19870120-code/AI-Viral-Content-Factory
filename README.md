...
GitHub 发布采用“非会员资产公开、会员内容占位”边界：只有目录名带 `（会员专享）` 的目录及其后代只保留占位文件；内容类型的 `memberOnly` 状态只控制工作台会员展示与业务权限，不参与发布判定。其余输入库、处理库、输出库、配图库资产和 OPPOSans 品牌字体均进入公开包。`node_modules`、运行态、审核/调度记录、历史恢复材料、临时文件、配图生成证据、本机路径与凭证始终不进入公开包。详见 [跨电脑安装与发布](00_系统说明/跨电脑安装与发布.md)。
