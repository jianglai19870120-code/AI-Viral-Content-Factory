...
首次安装前需安装 Node.js 20 LTS 与 Codex Desktop，然后执行 `python run.py install --dev`。该命令会根据 `03_工作台/vendor/codex-cli/package-lock.json` 运行 `npm ci` 安装锁定版 Windows Codex CLI；`node_modules` 是机器依赖，不进入 Git。
